...
public class MatlabFileWriter
{
    ...
    private void Flush()
    {
        ...
        fileWriter.Write(MatfileHelper.MatlabDataTypeNumber<ZLibStream>());
        using MemoryStream compressedStream = new();
        using ZLibStream zlibStream = new(compressedStream, CompressionMode.Compress);
        //zlibStream.Write(uncompressedStream);

        //    byte[] compressedBytes = compressedStream.ToArray();
        //byte[] compressedBuffer = stream.;
    ...
    public MatLabFileArrayWriter<T> OpenArray<T>(string varName, bool compress)
    {
        ...
        MatLabFileArrayWriter<T> arrayWriter;
        if(compress) {
            uncompressedStream = new MemoryStream();
            arrayWriter = new (varName, new BinaryWriter(uncompressedStream));
        } else {
            arrayWriter = new (varName, fileWriter);
        }
    ...
    public void Write(string name, object data, bool compress = true)
    {
        ...
        if(data.GetType().Equals
            (typeof(String))) {
                ...
                var charArrayWriter = OpenArray<char>(name, compress);
                charArrayWriter.AddRow(dataAsString.ToCharArray());
                charArrayWriter.FinishArray(typeof(char));
        }
        else{
            Type t;
            MatLabFileArrayWriter<object> arrayWriter;
